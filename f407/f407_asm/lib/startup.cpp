...
void (*vectors[])() __attribute__((section(".isr_vectors"))) = {
    ...
    Reset_Handler, NMI_Handler, HardFault_Handler, MemManage_Handler, BusFault_Handler, UsageFault_Handler, 0, 0, 0, 0,
...
void __libc_init_array(void) // static initialization constructors function
{
    int count;
    int i;
...
void __libc_fini_array(void) //!< destructors not usefull in microcontrollers
{
    int count;
    int i;
