...
{  
  if (epnum == 0) {
      ...
      /////////////// RECEIVE ////////////////////
      for(int i = 0; i < hpcd->OUT_ep[epnum].xfer_count; i++) {
        buf[i] = hpcd->OUT_ep[epnum].xfer_buff[i - hpcd->OUT_ep[epnum].xfer_count];
      }
      StartPlay = buf[0];
      Freq = (buf[1] << 8) + buf[2];
      
    //  ((USB_OTG_INEndpointTypeDef *)(USB2_OTG_FS_PERIPH_BASE + USB_OTG_IN_ENDPOINT_BASE + ((1) * USB_OTG_EP_REG_SIZE)))->DIEPCTL
    //   |= (USB_OTG_DIEPCTL_CNAK | USB_OTG_DIEPCTL_EPENA);
      //(void)USB_EPStartXfer(hpcd->Instance, &hpcd->OUT_ep[epnum], (uint8_t)hpcd->Init.dma_enable);
      (void)USBD_LL_PrepareReceive((USBD_HandleTypeDef*)hpcd->pData, epnum,
                                  (uint8_t*)hpcd->OUT_ep[epnum].xfer_buff, 0);//hpcd->OUT_ep[epnum].xfer_count);
      uint8_t x = buf[0];
  }
}
...
#endif /* USE_HAL_PCD_REGISTER_CALLBACKS */
{
  if (epnum == 0) {
    ...
    /////////////   to send back  ////////////////////////
    uint8_t buf[64] = {0};
    USBD_LL_Transmit((USBD_HandleTypeDef*)hpcd->pData, 0x81, buf, 64);
  }
}
