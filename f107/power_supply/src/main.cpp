...
#include "main.h"
#include <cmath>
void *__dso_handle = nullptr; // dummy "guard" that is used to identify dynamic shared objects during global
                              // destruction. (in fini in startup.cpp)
...
int main() {
    ...
    InterruptClass<TIM4_IRQn> subscriber;
    Adder adder;
    subscriber.addInterruptClass(&adder);
    subscriber.setVector();
    __enable_irq();
    SpiSlave spiSlave;
    while (1) {
        font.intToChar(adder.i);
